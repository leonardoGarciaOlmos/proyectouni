...
!function ( $ ) {
  ...
  , show: function () {
		...
		, matcher: function ( value ) {
			return ~value.toLowerCase().indexOf(this.query.toLowerCase()) && (that.inValues(value) == -1 || that.options.allowDuplicates) && (that.filter(value))
		  }
  ...
  , filter: function (value){
		if (!this.options.filterGlobal)
			return true;
		if (this.options.caseInsensitive) {
			var index = true;
			$.each(data_filter, function (indexInArray, valueOfElement) {
  ...
  ,beforeadd: function( value ){
  	if ($.inArray( value, data_filter)!= -1){
  		return -1;
  	}else{
  		return $.inArray( value, this.options.source);
  	}
