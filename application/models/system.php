<?php
	...
	*/
	class System extends MY_Model{
		...
		public function loadSyatemsAndRoles()
		{
			$data = array();
			$aux = $this->dsystem->getSystemsAndRoles();
			$validId = array();
			foreach( $aux as $key => $value){
				if( array_search($value['id'],$validId) ===false ){
					$validId[] =  $value['id'];
					$data[$key]['id'] = $value['id'];
					$data[$key]['system'] = $value['system'];
					$data[$key]['alias'] = $value['alias'];
					$cont = 0;
					foreach( $aux as $i => $v){
						if( $value['id'] == $v['id'] ){
							//$data[$key] = array();
							$data[$key]['roles'][$cont]['name'] = $v['name'];
							$data[$key]['roles'][$cont++]['id'] = $v['role_id'];
						}
					}
				}
			}
